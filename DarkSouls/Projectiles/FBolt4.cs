...
{
    class FBolt4 : ModProjectile
    {
        public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Bolt 4");
		...
		{
            if (Main.netMode != 2 && projectile.ai[0] <= 3)
            {
				int proj = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, 0, 0, mod.ProjectileType("FBolt4"), projectile.damage, 8f, projectile.owner,projectile.ai[0]+1);
		...
		public override bool PreDraw(SpriteBatch sb, Color lc)
		{
			...
			Texture2D Bolt2 = ModLoader.GetTexture("DarkSouls/Projectiles/FBolt4_2");
			Texture2D Bolt3 = ModLoader.GetTexture("DarkSouls/Projectiles/FBolt4_3");
			Texture2D Bolt4 = ModLoader.GetTexture("DarkSouls/Projectiles/FBolt4_4");
			...
			{
				sb.Draw(Bolt4, drawPos, new Rectangle(0, frameY, tex.Width, frameH), lc, p.rotation, origin, p.scale, fx, 0f);
				return false;
			}
			return true;
		}
