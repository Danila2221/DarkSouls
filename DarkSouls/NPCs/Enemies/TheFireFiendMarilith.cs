...
{
    public class TheFireFiendMarilith : ModNPC
    {
        float customAi1;
        int OTimeLeft = 2000;
        ...
        {
            DisplayName.SetDefault("Marilith, the Fire Fiend");
                        ...
                        if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                        {
                            if (Main.rand.Next(90) == 1)
                            {
                                ...
                                if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                                {
                                    ...
                                    speedY *= num51;
                                    int damage = 55;
                                    if (Main.expertMode) { damage = 75; }
                                    int type = mod.ProjectileType("Fire2Ball");
                            ...
                            if (Main.rand.Next(225) == 1)
                            {
                                ...
                                if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                                {
                                    ...
                                    speedY *= num51;
                                    int damage = 70;
                                    if (Main.expertMode) { damage = 100; }
                                    int type = mod.ProjectileType("Fire3Ball");
                                    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
                                    Main.projectile[num54].timeLeft = 100;
                                    Main.projectile[num54].aiStyle = -1;
                                    customAi1 = 1f;
                            ...
                            if ((Main.rand.Next(950) == 1) || (Main.rand.Next(700) == 1 && Main.expertMode))
                            {
                                float num48 = 8f;
                                float aNV1;
                                //int aNV2;
                            ...
                            if ((Main.rand.Next(350) == 1) || (Main.rand.Next(200) == 1 && Main.expertMode))
                            {
                                float num48 = 8f;
                                float aNV1;
                                //int aNV2;
        ...
        {
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore1"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore2"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore3"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore4"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore5"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore6"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore7"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore8"), 1.0f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/OmnirsFireFiendMarilithGore9"), 1.0f);
            Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsFireCrystal"));
            if (Main.rand.Next(2) == 0)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsFireFiendMarilithTrophy"));
            }
            int kLoot = (Main.rand.Next(7));
            if (kLoot == 0)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsFire3Tome"));
            }
            if (kLoot == 1)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsCure3Tome"));
            }
            if (kLoot == 2)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsHeal3Tome"));
            }
            if (kLoot == 3)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsDefender"));
            }
            if (kLoot == 4)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsKillerBow"));
            }
            if (kLoot == 5)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsDragonLance"));
            }
            if (kLoot == 6)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("OmnirsFlameSabre"));
            }
        }
    }
}
