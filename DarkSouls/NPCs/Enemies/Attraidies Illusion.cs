...
{
    public class AttraidiesIllusion : ModNPC
    {
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            return (!Main.hardMode && spawnInfo.player.ZoneUnderworldHeight) ? 0.07f : 0f;

            return (!Main.hardMode && spawnInfo.player.ZoneDungeon) ? 0.05f : 0f;

            return (Main.hardMode && spawnInfo.player.ZoneDungeon) ? 0.02f : 0f;

            return (Main.hardMode && spawnInfo.player.ZoneJungle) ? 0.01f : 0f;
        }
        ...
        public override void AI()
        {
            ...
            //if (Main.netMode != 2)
            //{
            if (npc.ai[0] >= 12 && npc.ai[2] < 6) //2 was 12
            {
                float num48 = 3f;
                ...
                else
                {
                    ...
                    while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
                    {
                        ...
                        { // (tp_x_target,m) is block under its feet I think
                            if ((m < target_y_blockpos - 15 || m > target_y_blockpos + 15 || tp_x_target < target_x_blockpos - 15 || tp_x_target > target_x_blockpos + 15))
            ...
            {
                if (Main.rand.Next(4) == 0) //1 in 2 chance boss will use attack when it flies down on top of you
                {
                    float num48 = 2f;
