...
namespace DarkSouls.NPCs.Enemies
{
	...
	public class AttraidiesMimic : ModNPC
	{
...
for (int num36 = 0; num36 < 20; num36++)
{
	...
	{
	int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float) Math.Sin(randomrot+((360/13)*(1+num36))*3),(float) Math.Cos(randomrot+((360/13)*(1+num36))*3), mod.ProjectileType("AntiMatterBlast"), 50, 0f, Main.myPlayer, npc.whoAmI);
	}
	genericTimer2 = 0;
	}
	...
	if (genericTimer2 >= 20)
	{
		...
		int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float)((Math.Cos(rotation) * 0.5)*-1),(float)((Math.Sin(rotation) * 0.5)*-1), mod.ProjectileType("AntiMatterBlast"), 50, 0f, 0);
		genericTimer2 = 0;
	}
	}
...
if (npc.velocity.X > 1.5f) npc.frame.Y = num;
	...
	if (ShieldBroken)
	{
	if (npc.alpha > 4) npc.alpha -= 1; //4 was 40, 100 was 200
	if (npc.alpha < 4) npc.alpha += 1;
	}
	else
	{
