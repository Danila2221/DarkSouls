...
namespace DarkSouls.NPCs.SuperHard
{
    public class AbyssWalker : ModNPC
    {
        ...
        public override void AI()  //  warrior ai
        {
            ...
            int projectile_id = mod.ProjectileType("SpellMeteor"); // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
            ...
            if (standing_on_solid_tile)  //  if standing on solid tile
            {
                ...
                if (Main.tile[x_in_front, y_above_feet - 1].type == 10 && can_pass_doors)
                ...
                else // standing on solid tile but not in front of a passable door
                {
                    ...
                    {  //  moving forward
                        if (Main.tileSolid[(int)Main.tile[x_in_front, y_above_feet - 2].type])
                        { // 3 blocks above ground level(head height) blocked
                            if (Main.tileSolid[(int)Main.tile[x_in_front, y_above_feet - 3].type])
                        ...
                        else if (Main.tileSolid[(int)Main.tile[x_in_front, y_above_feet - 1].type])
                        { // 2 blocks above ground level(mid body height) blocked
                            npc.velocity.Y = -6f; // jump with power 6 (for 2 block steps)
                            npc.netUpdate = true;
                        }
                        else if (Main.tileSolid[(int)Main.tile[x_in_front, y_above_feet].type])
                        { // 1 block above ground level(foot height) blocked
                            npc.velocity.Y = -5f; // jump with power 5 (for 1 block steps)
                            npc.netUpdate = true;
                        }
                        else if (npc.directionY < 0 && jump_gaps && (!Main.tileSolid[(int)Main.tile[x_in_front, y_above_feet + 1].type]) && (!Main.tileSolid[(int)Main.tile[x_in_front + npc.direction, y_above_feet + 1].type]))
            ...
            if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
            {
                ...
                int tp_counter = 0;
                bool flag7 = false;
                if (Math.Abs(npc.position.X - Main.player[npc.target].position.X) + Math.Abs(npc.position.Y - Main.player[npc.target].position.Y) > 2000000f)
                ...
                while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
                {
                    ...
                    { // (tp_x_target,m) is block under its feet I think
                        if ((m < target_y_blockpos - 6 || m > target_y_blockpos + 6 || tp_x_target < target_x_blockpos - 6 || tp_x_target > target_x_blockpos + 6))
