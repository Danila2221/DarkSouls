...
public void ChangeFirePhase(int a)
{
	...
	{
		if (Main.npc[num36].ai[1] == npc.whoAmI && Main.npc[num36].type == mod.NPCType("DamnedSoul"))
		{
	...
	{
	int rotball = NPC.NewNPC((int) ((npc.position.X+(npc.width/2)-(Main.npc[num36].width))+Math.Sin(npc.rotation+((360/10)*(1+num36)))*300), (int) ((npc.position.Y+(npc.height/2)-(Main.npc[num36].height))+Math.Cos(npc.rotation+((360/10)*(1+num36)))*300), mod.NPCType("Damned Soul"), 0);
	...
	for (int num36 = 0; num36 < 200; num36++)
	{
		if (Main.npc[num36].ai[1] == npc.whoAmI && Main.npc[num36].type == mod.NPCType("Damned Soul"))
		{
	...
	NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), mod.NPCType("Dark Dragon Mask"), 0);
	npc.active = false;
	}
	}
}
